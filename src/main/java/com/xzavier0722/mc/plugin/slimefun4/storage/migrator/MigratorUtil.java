...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            ...
            if (Files.exists(backupPath, LinkOption.NOFOLLOW_LINKS)) {
                Slimefun.logger().log(Level.WARNING, "Backup data already exists; skipping backup");
                return true;
            }
                ...
                try (var fs = Files.walk(src).filter(path -> !Files.isDirectory(path))) {
                    fs.forEach(path -> {
                        ...
                        try {
                            ...
                            Slimefun.logger().log(Level.WARNING, "Problem backing up old data " + dir.getName(), e);
                        }
                    });
                }
            }
    ...
    protected static void deleteOldFolder(File dir) {
        try {
            ...
            Slimefun.logger().log(Level.WARNING, "Problem deleting folder " + dir.getAbsolutePath(), e);
        }
    }
