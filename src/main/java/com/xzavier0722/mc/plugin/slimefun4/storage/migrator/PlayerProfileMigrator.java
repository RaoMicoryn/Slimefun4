    ...
    public MigrateStatus migrateData() {
        ...
        for (File file : listFiles) {
            ...
            try {
                ...
                if (!p.hasPlayedBefore()) {
                    Slimefun.logger()
                            .log(Level.INFO, "Data for a player who never joined the server was skipped: " + uuid);
                    total--;
                    continue;
                }
                ...
                Slimefun.logger()
                        .log(Level.WARNING, "Invalidly named player data file detected: '" + file.getName() + "'");
        ...
        if (MigratorUtil.createDirBackup(playerFolder)) {
            Slimefun.logger()
                    .log(
                            Level.INFO,
                            "Successfully migrated {0} player data files! Pre-migration data is stored in ./data-storage/Slimefun/old_data",
                            migratedCount);
            try {
                ...
                result = MigrateStatus.FAILED;
                Slimefun.logger()
                        .log(Level.WARNING, "Failed to delete the old player data folder; delete it manually", e);
            }
        }
