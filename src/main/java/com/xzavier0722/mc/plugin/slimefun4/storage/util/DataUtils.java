    ...
    public static byte[] serializeItemStackBytes(ItemStack itemStack) {
        ...
        try {
            ...
            if (!Slimefun.getConfigManager().isBypassItemLengthCheck()
                ...
                throw new IllegalArgumentException("Oversized item detected; contact the item plugin developer: "
            ...
            Slimefun.logger()
                    .log(Level.SEVERE, "An error occurred while serializing an item; storing an empty value", e);
            return new byte[0];
        }
    }
        ...
        try {
            ...
            if (result.getType().isAir()) {
                Slimefun.logger()
                        .log(
                                Level.SEVERE,
                                "Failed to deserialize an item from the database! The item cannot be displayed.");
            }
