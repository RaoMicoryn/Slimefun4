...
public final class WikiUtils {
    ...
    /**
     * Reads the addon's wiki.json and sets the Wiki button for items.
     *
     * @param addon {@link SlimefunAddon} instance of the addon
     */
    ...
    /**
     * Reads the addon's wiki.json and sets the Wiki button for items.
     * Allows modifying the page URL.
     *
     * @param plugin {@link SlimefunAddon} instance of the addon
     * @param formatter Modifies the page URL
     */
    public static void setupJson(Plugin plugin, Function<String, String> formatter) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            plugin.getLogger()
                    .log(
                            Level.INFO,
                            MessageFormat.format("Loaded wiki pages for {1} items in {0}", plugin.getName(), count));
        } catch (Exception e) {
            plugin.getLogger()
                    .log(Level.SEVERE, MessageFormat.format("Failed to load wiki.json for {0}", plugin.getName()), e);
        }
    }
}
